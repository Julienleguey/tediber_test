    ...
    <div id="press-cards">
      ...
      <div class="cards-container">
        <div class="button-container">
          <button class="button before" v-on:click="prevCard"/>
        </div>
        <transition-group name="switch" tag="div" class="cards-container">
          <div v-for="card in displayedCards" class="card switch-card" v-bind:key="card.title">
            <div class="card-main">
              <div class="ribbon">
            ...
            <div class="card-bottom">
              <a v-bind:href="card.url" target="_blank" class="card-link">{{ linkName }}</a>
            </div>
          </div>
        </transition-group>
        <div class="button-container">
          <button class="button after" v-on:click="nextCard"/>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    mounted() {
        this.press.forEach( card => {
          if(this.categoryType.includes(card.type)) {
            this.allCards.push(card);
          }
        });
        // change "i < ?" to change the size of the displayedCards array
        for (let i = 0; i < 3; i++) {
          this.displayedCards.push(this.allCards[i]);
        }
    },
    methods: {
      ...
      nextCard: function() {
        ...
        for (let i = 0; i < this.allCards.length; i++) {
          if (this.allCards[i].title === this.displayedCards[1].title) {
            if (i+1 < this.allCards.length) {
              this.displayedCards.push(this.allCards[i+1]);
            } else {
              this.displayedCards.push(this.allCards[0]);
            }
            break;
          }
        }
      ...
      prevCard: function() {
        ...
        for (let i = 0; i < this.allCards.length; i++) {
          if (this.allCards[i].title === this.displayedCards[0].title) {
            if (i > 0) {
              this.displayedCards.unshift(this.allCards[i-1]);
            } else {
              this.displayedCards.unshift(this.allCards[this.allCards.length-1]);
            }
            break;
          }
        }
